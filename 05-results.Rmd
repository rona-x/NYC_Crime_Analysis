...
ggparcoord(dv36,columns = 2:25,groupColumn = "County",scale = "globalminmax")+
  ...
  ggtitle("Total Number of Crimes in Different Time Range of Different County ")+
...
```{r CountyC1, fig.width=5,fig.height=10}
library(tidyverse)
library(Lock5withR)
library(vcd)
library(grid)
...
ggparcoord(dh3,columns = 2:24,groupColumn = "County",scale = "globalminmax")+
  ...
  theme(legend.position="bottom")+
  labs(x = "Bias",y= " Count")+
  ggtitle('Bias in Different County')+
  theme(plot.title = element_text(hjust = 0.5))
  
```


Pick the top 5 bias in the whole area shown in the last graph and check the situation in each plot. It doesn't mean that these 5 bias should be the top 5 bias in each county.
```{r}
