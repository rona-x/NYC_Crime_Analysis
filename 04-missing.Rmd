...
createNAplot <- function(data, percent = TRUE) {
  ...
  } else { p1 <- p1 +  ylab("num rows missing:") }
 ...
 # assemble plots
  ...
  patchwork + plot_annotation(title = 'Missing Value Patterns')+ theme(plot.title = element_text(hjust = 0.5))

}

```
```{r}
createNAplot(d4,FALSE)
```
...
We can see that the number of missing values is too small compared to that of other values. Hence we drop all NA.
```{r}
d5 <- drop_na(d4)
write.csv(d5, file = "Data/NYPD_CRIME_FINAL.csv")
```
